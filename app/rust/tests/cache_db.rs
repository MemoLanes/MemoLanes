...
fn basic() {
    ...
    assert_eq!(
        cache_db
            .get_journey_cache_or_compute(Some(&journey_kind), || { Ok(journey_bitmap.clone()) })
            .unwrap(),
        journey_bitmap
    );
    ...
    assert_eq!(
        cache_db
            .get_journey_cache_or_compute(Some(&journey_kind), || panic!("Should not be called"))
            .unwrap(),
        journey_bitmap
    );
    // test clear cache
    ...
    assert_eq!(
        cache_db
            .get_journey_cache_or_compute(Some(&journey_kind), || { Ok(JourneyBitmap::new()) })
            .unwrap(),
        JourneyBitmap::new()
    );
