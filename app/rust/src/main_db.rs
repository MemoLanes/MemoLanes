...
impl Txn<'_> {
    ...
    pub fn copy_journey(&mut self, id: &str) -> Result<String> {
        ...
        // Do not run GPS postprocessing again on an existing track.
        self.db_txn.execute(
            "INSERT INTO journey (id, journey_date, timestamp_for_ordering, type, journey_kind, header, data)
             SELECT ?1, journey_date, timestamp_for_ordering, type, journey_kind, ?2, data
