...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let token = server.lock().unwrap().register_map_renderer(Arc::new(Mutex::new(map_renderer_static)));
    ...
    let token = server.lock().unwrap().register_map_renderer(map_renderer_arc);
