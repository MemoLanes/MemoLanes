export class JourneyCanvasLayer {
  ...
  render() {
    ...
    if (
      !this.arraysEqual(this.currentTileRange, tileRange) ||
  ...
  renderTileRange(tileRange, zoom) {
    ...
    if (left > right || top > bottom) {
      console.error(
        `Invalid tile range: left=${left}, right=${right}, top=${top}, bottom=${bottom}`,
      );
      return;
    }
  ...
  renderTile(x, y, z) {
    try {
      const imageBufferRaw = this.journeyBitmap.get_tile_image(
        BigInt(x),
        BigInt(y),
        z,
      );
