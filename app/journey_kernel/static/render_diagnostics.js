...
function trySetup() {
    ...
    const hasHttpEndpoint = window.EXTERNAL_PARAMS.http_endpoint || window.EXTERNAL_PARAMS.httpEndpoint;
    ...
    if (window.EXTERNAL_PARAMS.flutter_channel) {
        ...
        if (window.EXTERNAL_PARAMS.cgi_endpoint === "flutter") {
            ...
            const flutterChannel = window.EXTERNAL_PARAMS.flutter_channel || 'RenderDiagnosticsChannel';
    ...
    const flutterChannel = flutterRequester ? flutterRequester.channelName : 'Not configured';
    ...
    if (flutterRequester) {
        log(`Flutter IPC ready: ${flutterRequester.channelName} (available: ${flutterRequester.getStatus().channelAvailable})`);
...
async function makeRequest() {
    ...
    if (isIpc) {
        if (!flutterRequester) {
            log(`Request #${requestId}: IPC Channel - SKIPPED - Flutter requester not configured`);
            return;
        }
        ...
        if (!httpRequester) {
            log(`Request #${requestId}: HTTP (${path}) - SKIPPED - HTTP requester not configured`);
            return;
        }
...
function startTest() {
    ...
    log(`Test started - alternating between ${configuredEndpoints.join(' and ')} every 5 requests`);
...
function clearLog() {
    ...
    if (httpRequester || flutterRequester) {
        const httpStatus = httpRequester ? httpRequester.cgiEndpoint : 'Not configured';
        const ipcStatus = flutterRequester ? 
            (flutterRequester.getStatus().channelAvailable ? 'Available' : 'Not Available') : 
            'Not configured';
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (window.EXTERNAL_PARAMS.cgi_endpoint) {
        log(`Page loaded with hash parameters: ${JSON.stringify(window.EXTERNAL_PARAMS)}`);
    } else {
