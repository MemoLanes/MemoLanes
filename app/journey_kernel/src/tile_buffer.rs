...
impl TileBuffer {
    ...
    pub fn from_journey_bitmap(
        ...
        for tile_y in y..(y + height) {
            for tile_x in x..(x + width) {
                ...
                let tile_x_rounded = ((tile_x % zoom_coefficient) + zoom_coefficient) % zoom_coefficient;
                ...
                for (px, py) in pixels {
                    ...
                    if px >= 0 && px < (1 << buffer_size_power) && py >= 0 && py < (1 << buffer_size_power) {
    ...
    pub fn contains_tile(&self, tile_x: i64, tile_y: i64) -> bool {
        ...
        x_offset >= 0 && x_offset < self.width &&
        y_offset >= 0 && y_offset < self.height
    }
    ...
    pub fn get_tile_pixels(&self, tile_x: i64, tile_y: i64, tile_z: i16, buffer_size_power: i16) -> Option<&Vec<(u16, u16)>> {
        println!("get_tile_pixels: tile_x: {}, tile_y: {}, tile_z: {}, buffer_size_power: {}", tile_x, tile_y, tile_z, buffer_size_power);
        println!("self.z: {}, self.buffer_size_power: {}", self.z, self.buffer_size_power);
        // Check if the tile is within this buffer and at the correct zoom level
        if tile_z != self.z || buffer_size_power != self.buffer_size_power || !self.contains_tile(tile_x, tile_y) {
            return None;
        }
    ...
    pub fn to_bytes(&self) -> Result<Vec<u8>, String> {
        bincode::serialize(self)
            .map_err(|e| format!("Failed to serialize TileBuffer: {}", e))
    }
}
